rule all_methylation:
    input:
        # Verkko-only: the "asm" ref type maps to assembly/output/verkko/{sample}/
        # below, which hifiasm-tool finished_samples entries don't have.
        expand("analysis_other/mod/{sample}/{sample}.{ref}.modkit_summary.tsv", sample = [s for s in finished_samples if s in asm_samples], ref = ["asm", "ref", "ref.hp1", "ref.hp2"]),
        expand("analysis_other/mod/{sample}/{sample}.{ref}.methyl.bed", sample = [s for s in finished_samples if s in asm_samples], ref = ["asm", "ref"]),
        expand("analysis_other/mod/{sample}/{sample}.{ref}.bw", sample = [s for s in finished_samples if s in asm_samples], ref = ["asm", "ref"]),
...
rule collect_methylation_summary:
    input:
        # ref.hp1/hp2 need the verkko-only haplotagging chain (see all_methylation)
