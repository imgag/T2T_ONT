...
rule create_PE_from_PoreC:
    input:
        "assembly/input/{asm}/{asm}.POREC.fastq.gz"
    output:
        "assembly/input/{asm}/{asm}.POREC_R1.fa.gz",
        "assembly/input/{asm}/{asm}.POREC_R2.fa.gz"
    log:
        "logs/create_PE_from_PoreC/{asm}.log"
    benchmark:
        "runtimes/create_PE_from_PoreC/{asm}.txt"
    conda:
        "../env/py_samtools.yml"
    params:
        script="workflow/scripts/digestFasta2PE.py",
        cutsite="CATG",
        minlen=200,
        maxFragUse=5
    shell:
        """
        OUT="assembly/input/{wildcards.asm}/{wildcards.asm}.POREC"

        samtools fasta {input} 2>{log} \
        | python {params.script} \
            --cutsite {params.cutsite} \
            --maxFragUse {params.maxFragUse} \
            --minlen {params.minlen} \
            --outpref $OUT \
            >>{log} 2>&1
        
        bgzip ${{OUT}}_R1.fa >>{log} 2>&1
        bgzip ${{OUT}}_R2.fa >>{log} 2>&1
        """