...
def find_input_datasets(wc, print_debug=True):
    ...
    if print_debug: print(f"{wc.dataset} | {wc.type}")
    ...
    if str(wc.type).startswith("HQ_combined"):
        ...
        wc_type = "HQ_combined"
        if print_debug: print("--> split combined dataset")
        ...
        for wc_type, elements in elements.items():
            ...
            for e in elements:
                # Is a folder
                if os.path.isdir(e):
                    if print_debug: print(f"--> transform input folder: {e}")
                ...
                # Is a file
                    ...
                    match wc_type:
                        case "HQ_herro":
                            ...
                            if print_debug: print(f"--> transform herro path: {e}")
                            files.append(f)
                        case "HQ_duplex":
                            files.append(e)
                        case "UL":
                        ...
                        case "POREC":
                            files.append(e)
                        case _:
                            if print_debug: print(f"Unrecognized dataset type {wc.type} for {e}")
                            files.append(e)
    
    if not files:
        if print_debug: print("Files empty: files generated by other rule")
    else:
        if print_debug: print("Updated files:")
        if print_debug: pprint(files)
...
def input_isbam(wc):
    ...
    if all(f.endswith(".bam") for f in files):
        ...
        raise ValueError("Inconsistent file types: some files have .bam ending and some don't. Cannot merge")
