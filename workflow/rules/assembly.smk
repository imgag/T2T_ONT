def get_assembly_input(wc):
    files = {"porec": [], "porec_pe": [], "apk": [], "trio_kmers": [], "ul": [], "hq": [], }
    ...
    if "POREC" in datasets[s_d]:
        ...
        if asm[wc.asm].get("hifiasm", "") == "ont_porec":
            files["porec_pe"] = [
                f"assembly/input/{s_d}/{s_d}.POREC_R1.fa.gz",
                f"assembly/input/{s_d}/{s_d}.POREC_R2.fa.gz",
            ]
...
rule verkko_scaffold:
    input:
        ul=lambda wc: get_assembly_input(wc).get("ul"),
        hq=lambda wc: get_assembly_input(wc).get("hq"),
        porec=lambda wc: get_assembly_input(wc).get("porec") if not asm[wc.asm].get("trio_phasing", False) else [],
    ...
    output:
        ...
        done="assembly/output/verkko/{asm}/create_scaffold.done",
    conda:
        "../env/verkko.yml"
    group:
        "verkko"
    ...
    threads: 72
    params:
        scaffold_opts=lambda wc: (
            f"--hap-kmers {' '.join(get_assembly_input(wc).get('trio_kmers', []))} trio"
            if asm[wc.asm].get("trio_phasing", False)
            else f"--porec {get_assembly_input(wc).get('porec', '')}"
        ),
        dryrun="--dryrun" if config["verkko_dryrun"] else "",
        skip_verkko_polish=lambda wc: "--no-correction"
        if asm[wc.asm].get("skip_verkko_polish", True)
    ...
    try:
        ...
        try:
            opts = opts.format(
                ul = input.ul,
                hq=input.hq,
                porec_r1 = input.porec_pe[0],
                porec_r2 = input.porec_pe[1])
        except:
            opts = opts.format(
                ul = input.ul,
                hq=input.hq)
...
rule hifiasm:
    input:
        ...
        porec_pe=lambda wc: get_assembly_input(wc).get("porec_pe", []),
    output:
        primary_gfa="assembly/output/hifiasm/{asm}/{asm}.{suf}.p_utg.gfa",
        hap1_gfa="assembly/output/hifiasm/{asm}/{asm}.{suf}.hap1.p_ctg.gfa",
        hap2_gfa="assembly/output/hifiasm/{asm}/{asm}.{suf}.hap2.p_ctg.gfa",
    wildcard_constraints:
        suf = "[^.]+",
        asm = "[^.]+"
    log:
        "logs/hifiasm/{asm}.{suf}.log",
    benchmark:
        "runtimes/hifiasm/{asm}.{suf}.hifiasm.txt"
    threads: 72
    resources:
        mem_gb = 600
    params:
...
def get_hifiasm_outputs(wc):
    # Case 1: No Phasing, Only UL + HQ
    if not asm[wc.asm].get("trio_phasing", False):
        suf = "bp"
    else:
        print(f"Unknown hifiasm assembly type for {wc.asm}")
        return {"primary_gfa":"", "hap1_gfa":"", "hap2_gfa":""}
    # Case 2: Trio Phasing
    if asm[wc.asm].get("hifiasm", "") in ["ont_trio", "herro_trio"]:
        suf = "dip"
    # Case 3: PoreC Phasing
    elif asm[wc.asm].get("hifiasm", "") in ["ont_porec"]:
        suf = "hic"
    ...
    return{
        "primary_gfa" : f"assembly/output/hifiasm/{wc.asm}/{wc.asm}.{suf}.p_utg.gfa",
        "hap1_gfa" : f"assembly/output/hifiasm/{wc.asm}/{wc.asm}.{suf}.hap1.p_ctg.gfa",
        "hap2_gfa" : f"assembly/output/hifiasm/{wc.asm}/{wc.asm}.{suf}.hap2.p_ctg.gfa",
    }

rule hifiasm_to_fasta:
    input:
        unpack(get_hifiasm_outputs)
    output:
