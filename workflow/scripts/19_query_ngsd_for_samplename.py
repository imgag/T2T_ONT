...
def read_existing_data(file_path: str) -> pd.DataFrame:
    ...
    return pd.DataFrame(columns=['name_ngsd', 'name_external', 'project_name', 'run_flowcell_id'])
...
def merge_data(existing_df: pd.DataFrame, new_df: pd.DataFrame) -> pd.DataFrame:
    ...
    existing_df['composite_key'] = existing_df.apply(lambda x: f"{x['name_ngsd']}_{x['run_flowcell_id']}", axis=1)
    new_df['composite_key'] = new_df.apply(lambda x: f"{x['name_ngsd']}_{x['run_flowcell_id']}", axis=1)
