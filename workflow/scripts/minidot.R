...
## debug
#setwd("/mnt/storage3b/projects/no_ngsd/ahthapp1_T2T_ONT/")
#args <- data.frame('i' = "assembly_qc/published_chr19/haplotype1.mapped_T2T.paf", 'l'= "assembly_qc/published_chr19/scaffold_lengths.haplotype1.txt");

## * read paf and len
paf <- fread(args$i, sep = "\t", fill =T,  select = c(1:12))
write("finished reading paf", stderr())
len <- read.table(args$l) #, stringAsFactor=FALSE)
write("finished reading lengths", stderr())
