...
if (length(args) < 9) {
    stop(paste(
        ...
        "<nucflag_status> <rm_satellites> <rm_telo> <telo> <output_prefix>",
        "\n\nArguments:",
        ...
        "\n  rm_telo:         RepeatMasker telomeres BED file",
...
# Load telomeres from RepeatMasker
dt_rmtelo <- read_tsv(rm_telo, 
                      col_names = c("contig", "start", "end", "family", "name"), 
                      col_types = "cddcc", show_col_types = FALSE)
dt_rmtelo_plot <- join_bed(dt_rmtelo, dt)
...
ggsave(plot = p1, filename = paste0(output_prefix, ".gaps.with_unassigned.png"), 
       height = 15, width = 8)
...
ggsave(plot = p2, filename = paste0(output_prefix, ".gaps.clean.png"), 
       height = 15, width = 8)
...
ggsave(plot = p3, filename = paste0(output_prefix, ".nucflag.clean.png"), 
       height = 15, width = 8)
...
cat("Output files:\n")
cat("  -", paste0(output_prefix, ".gaps.with_unassigned.png\n"))
cat("  -", paste0(output_prefix, ".gaps.clean.png\n"))
cat("  -", paste0(output_prefix, ".nucflag.clean.png\n"))
