...
main_colour <- "#ffd900"  # Gold for highlighted sample
background_alpha <- 0.3    # Semi-transparent for background
background_color <- "#999999"  # Medium grey for samples without metadata
samples_color <- "#111111"  # Dark grey for samples without metadata (more visible than background color)
sample_alpha <- 0.9        # Slightly transparent for our samples to show overlap
...
# ---- Group POP legend by SUPERPOP (label prefixing) ----
if (color_col == "POP") {
  # Create grouped label for legend
  pca_df <- pca_df %>%
    mutate(
      POP_GROUP = case_when(
        is.na(POP) ~ NA_character_,
        is.na(SUPERPOP) ~ POP,
        TRUE ~ paste0(SUPERPOP, " - ", POP)
      )
  )

  # Build palette with grouped labels
  pop_to_super <- pca_df %>%
    filter(!is.na(POP), !is.na(SUPERPOP)) %>%
    distinct(POP, SUPERPOP)

  grouped_palette <- setNames(
    object = pop_palette,
    nm = sapply(names(pop_palette), function(pop) {
      sp <- pop_to_super$SUPERPOP[pop_to_super$POP == pop]
      if (length(sp) > 0) paste0(sp[1], " - ", pop) else pop
    })
  )

  color_col <- "POP_GROUP"
  color_palette <- grouped_palette
}
...
create_pca_plot <- function(df, pc_x, pc_y, title_base) {
  ...
  if (any(df$Type == "Background" & !df$OurSample & (is.na(df[[color_col]]) | !has_pop_data))) {
    p <- p + geom_point(
      ...
      aes(shape = SampleType),
      color = background_color,  # Dark grey for better visibility
      alpha = background_alpha, 
      size = background_size,
      stroke = 0.5
    )
  ...
  if (any(df$Type == "Background" & df$OurSample & (is.na(df[[color_col]]) | !has_pop_data))) {
    p <- p + geom_point(
      data = filter(df, Type == "Background" & OurSample & (is.na(.data[[color_col]]) | !has_pop_data)),
      aes(shape = SampleType),
      color = samples_color,  # Medium grey
      alpha = sample_alpha, 
      size = sample_size,
      stroke = 0.8
    )
  ...
  if (any(df$Type == "Main")) {
    p <- p + geom_point(
      ...
      aes(shape = SampleType),
      color = "grey20",  # Grey border, with highlighted fill
      fill = main_colour,
      size = highlight_size,
      stroke = 0.8,
      alpha = 1.0
