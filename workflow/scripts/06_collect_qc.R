...
if (test_mode) {
  ...
  input_t2t_scaffold <- snakemake@input[["t2t_scaffold"]]
...
message(paste("- FindT2T scaffold:", length(input_t2t_scaffold), "files"))
...
get_file_metadata <- function(file) {
  list(
    haplotype = str_extract(basename(file), "haplotype\\d|both"),  # Extract haplotype from filename
...
parse_t2t_scaffold <- function(files) {
  print(paste("Parse T2T scaffold statistics from ", files))
  results <- lapply(files, function(file) {
    metadata <- get_file_metadata(file)
    
    # Read the T2T scaffold file
    data <- read_tsv(file, col_names = TRUE, show_col_types = FALSE)
    
    # Count T2T contigs (classification == "contig")
    n_t2t_contig <- nrow(data %>% filter(classification == "contig"))
    
    # Count T2T scaffolds (classification == "scaffold")
    n_t2t_scaffold <- nrow(data %>% filter(classification == "scaffold"))
    
    # Create tibble with counts
    tibble(
      metric = c("n_t2t_contig", "n_t2t_scaffold"),
      value = c(n_t2t_contig, n_t2t_scaffold),
      haplotype = metadata$haplotype,
      asm_method = metadata$asm_method,
      asm_name = metadata$asm_name,
      source = "t2t_scaffold"
...
parsed_t2t_scaffold <- time_execution(parse_t2t_scaffold, input_t2t_scaffold, "T2T scaffold statistics parsing")
